...
let userId;
let gameId = '5a948885-fff2-4749-a3b5-9b58396fa545';
let sequence = 0;
...
const sendLocation = async (socket, packet) => {
    const protoMessages = getProtoMessages();
    const Packet = protoMessages.common.Packet;
    if (!Packet) {
        console.error('Packet 메시지를 찾을 수 없습니다.');
    ...
    // 패킷 타입 정보를 포함한 버퍼 생성
    const packetType = Buffer.alloc(PACKET_TYPE_LENGTH);
    packetType.writeUInt8(3, 0); // NORMAL TYPE
...
const updateLocation = (socket) => {
    x += 0.3;
    const packet = createPacket(6306, { gameId, x, y }, '1.0.0', 'game', 'LocationUpdatePayload');

    sendLocation(socket, packet);
};
...
client.connect(PORT, HOST, async () => {
    ...
    const successPacket = createPacket(52, { deviceId }, '1.0.0', 'initial', 'Packet');
